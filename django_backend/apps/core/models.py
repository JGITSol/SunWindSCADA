...
class WindTurbine(models.Model):
    ...
    rotor_diameter = models.FloatField(help_text='Rotor diameter in meters')
    nominal_power = models.FloatField(help_text='Nominal power in W')
    status = models.CharField(max_length=32, default='operational', 
                             choices=[
...
class WeatherData(models.Model):
    ...
    wind_direction = models.FloatField(help_text='Wind direction in degrees', null=True, blank=True)
...
class TurbineMeasurement(models.Model):
    ...
    """
    turbine = models.ForeignKey(WindTurbine, on_delete=models.CASCADE, related_name='measurements')
    timestamp = models.DateTimeField(auto_now_add=True)
    power_output = models.FloatField(help_text='Power output in W')
    wind_speed = models.FloatField(help_text='Local wind speed at turbine in m/s')
    rotor_speed = models.FloatField(help_text='Rotor speed in rpm', null=True, blank=True)
    blade_pitch = models.FloatField(help_text='Blade pitch angle in degrees', null=True, blank=True)
    nacelle_orientation = models.FloatField(help_text='Nacelle orientation in degrees', null=True, blank=True)
    grid_voltage = models.FloatField(help_text='Grid voltage in V', null=True, blank=True)
    grid_frequency = models.FloatField(help_text='Grid frequency in Hz', null=True, blank=True)
...
class GridComplianceCheck(models.Model):
    ...
    """
    turbine = models.ForeignKey(WindTurbine, on_delete=models.CASCADE, related_name='compliance_checks')
    timestamp = models.DateTimeField(auto_now_add=True)
    ...
    voltage_pu = models.FloatField(help_text='Voltage in per-unit', null=True, blank=True)
    duration = models.FloatField(help_text='Event duration in seconds', null=True, blank=True)
    frequency = models.FloatField(help_text='Frequency in Hz', null=True, blank=True)
