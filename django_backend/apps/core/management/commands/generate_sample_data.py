...
from django_backend.apps.core.models import WindTurbine, WeatherData, TurbineMeasurement, Scenario, GridComplianceCheck
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f'Generating {hours} hours of data with {interval}-minute intervals using {pattern} pattern')
    ...
    def generate_data(self, hours, interval, pattern_name):
        ...
        if pattern_name == 'random':
            pattern = random.choice(self.template['weather_patterns'])
        else:
            patterns = {p['name'].lower(): p for p in self.template['weather_patterns']}
            pattern = patterns.get(
                pattern_name, 
                next((p for p in self.template['weather_patterns'] if pattern_name in p['name'].lower()), 
        ...
        for i in range(num_points):
            ...
            if pattern['wind_pattern'] == 'diurnal':
                ...
                base_wind_speed = pattern['base_wind_speed'] + 2.0 * np.sin(np.pi * hour_of_day / 12.0)
            ...
            wind_speed = max(0, base_wind_speed + random.normalvariate(0, 1.0))
            wind_direction = random.uniform(0, 360)
            
            # Temperature varies with time of day
            temp_min, temp_max = pattern['temperature_range']
            temperature = temp_min + (temp_max - temp_min) * np.sin(np.pi * hour_of_day / 12.0)**2
            ...
            humidity = humidity_min + (humidity_max - humidity_min) * random.random()
    ...
    def generate_compliance_checks(self, turbines, hours):
        ...
        now = timezone.now()
        num_events = random.randint(3, 10)  # Random number of events
        ...
        for _ in range(num_events):
            ...
            # Random check type
            check_type = random.choice(['lvrt', 'hvrt', 'frequency', 'reactive_power'])
            ...
            if check_type == 'lvrt':
                ...
                voltage_pu = random.uniform(0.0, 0.9)
                
                # Get maximum allowed duration from curve
                max_duration = np.interp(
                    voltage_pu,
                    lvrt_curve['voltage_pu'],
                    lvrt_curve['duration_seconds']
                )
                
                # Random duration, sometimes exceeding the limit
                if random.random() < 0.8:  # 80% compliant
                    duration = random.uniform(0, max_duration * 0.9)
                    compliant = True
                else:
                    duration = random.uniform(max_duration * 1.1, max_duration * 2.0)
                    compliant = False
                ...
                max_duration = np.interp(
                    voltage_pu,
                    hvrt_curve['voltage_pu'],
                    hvrt_curve['duration_seconds']
                )
                ...
                frequency = random.uniform(47.0, 52.0)
                
                # Check compliance
                freq_limits = grid_compliance['frequency_limits']
                if freq_limits['min_continuous'] <= frequency <= freq_limits['max_continuous']:
                    compliant = True
                    details = {'status': 'within_continuous_limits'}
                elif freq_limits['min_temporary'] <= frequency <= freq_limits['max_temporary']:
                    compliant = True
                    details = {
                ...
                voltage_pu = random.uniform(0.9, 1.1)
