...
from django_backend.apps.core.models import WindTurbine, WeatherData, TurbineMeasurement, Scenario
...
class Command(BaseCommand):
    ...
    def _create_sample_data(self):
        ...
        for i in range(288):  # 5-minute intervals for 24 hours
            ...
            wind_speed = max(0, base_wind_speed + random.normalvariate(0, 0.8))
            wind_direction = random.uniform(0, 360)
            temperature = 15.0 + 5.0 * np.sin(np.pi * hour_of_day / 12.0) + random.normalvariate(0, 1.0)
